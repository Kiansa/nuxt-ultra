...
## Condition
- Apply when Auth is selected and Supabase is chosen as the provider.

## File Updates

### Path nuxt.config.ts
in case of using supabase auth, patch the following into the `supabase` config block:
```ts
supabase: {
    useSsrCookies: true,
    redirect: false,
...
in case of using supabase auth:
...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  if (to.meta.groups?.includes('protected') && !data?.claims) {
    console.log('Access to protected route denied. User not authenticated.')
    // save the intended route for redirect after login on session storage
    sessionStorage.setItem('redirectAfterLogin', to.fullPath)
    return await navigateTo(localePath('/get-started'))
  }
})
```
...
### Path: app/pages/get-started.vue
```vue
<script setup lang="ts">
...
const errMsg: Ref<any> = ref(undefined)
const confirmMsg: Ref<any> = ref(undefined)
  ...
  try {
    ...
    if (!isLogin.value) {
      ...
      if (data.user?.identities?.length === 0) {
        errMsg.value = 'auth.get_started_exists'
      }
      else {
        confirmMsg.value = 'auth.get_started_confirm'
      }
    }
    ...
    if (isLogin.value) {
      ...
      if (data?.user?.confirmed_at) {
        await navigateTo(localePath('/confirm'))
      }
      else {
        confirmMsg.value = 'auth.get_started_confirm'
      }
...
async function signInWithOAuth(provider: OAuthProvider) {
  const { error } = await supabase.auth.signInWithOAuth({
    provider,
    options: {
      redirectTo: `${config.public.siteUrl}${localePath('/confirm')}`,
    },
  })
  if (error) {
...
<template>
  <UAuthForm
    ...
    :schema="schema"
    :providers="providers"
    :title="isLogin ? $t('auth.login.head') : $t('auth.register.head')"
    :icon="isLogin ? 'i-lucide-lock' : 'i-lucide-user-plus'"
    :submit="{
      label: isLogin ? $t('auth.register.login') : $t('auth.login.signup'),
      color: 'primary',
      variant: 'subtle',
      loading: loading,
    }"
  ...
  >
    <template #description>
      ...
      >
        {{ isLogin ? $t('auth.login.signup') : $t('auth.register.login') }}
      </ULink>
    </template>
...
only if `@nuxtjs/supabase` is selected:
...
<template>
  <UPage>
    <UPageSection
      :description="$t('auth.login.signing')"
