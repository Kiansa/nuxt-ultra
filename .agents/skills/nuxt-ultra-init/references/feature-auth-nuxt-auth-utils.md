...
## Condition
- Apply when Auth is selected and Nuxt Auth Utils is chosen as the provider.

## File Updates

### Path nuxt.config.ts
- ask user to choose (multi select) oauth providers to support (google, github, etc) and then push it into the `auth.oauth` array in the config:
```ts
auth: {
    oauth: ['google'],
  },
...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  const { loggedIn, session, clear: clearSession, fetch: _fetchSession } = useUserSession()
  console.log('Current session:', session.value)
  console.log('loggedIn:', loggedIn.value)

  if (to.meta.groups?.includes('protected') && !loggedIn.value) {
    console.error('Access to protected route denied. User not authenticated.')
    // save the intended route for redirect after login on session storage
    // sessionStorage.setItem('redirectAfterLogin', to.fullPath)
    return await navigateTo(localePath('/login'))
  }
  // clear session if logged in more than 1 day ago
  if (to.meta.groups?.includes('protected') && session.value?.loggedInAt && session.value.loggedInAt <= Date.now() - 1 * 24 * 60 * 60 * 1000) {
  ...
  // block access to login and register pages if already logged in
  if ((to.path === localePath('/login') || to.path === localePath('/register')) && loggedIn.value) {
...
<template>
  <div class="relative flex min-h-screen items-center justify-center">
    ...
    />
    <StarsBg />
    <CommonLogo class="absolute top-8 left-8" />
    <UColorModeButton
...
const credential_id = ref<string>('')
...
const providers = [{
  ...
  onClick: () => {
    toast.add({ title: t('auth_google'), description: t('auth_login_with_google') })
  },
}, {
  label: t('auth_github'),
  icon: 'i-simple-icons-github',
  onClick: () => {
    toast.add({ title: t('auth_github'), description: t('auth_login_with_github') })
  },
}]

const schema = z.object({
  email: z.email(t('auth_email_invalid')),
  password: z.string(t('auth_password_required')).min(8, t('auth_password_min')),
})
...
async function onSubmit(payload: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    if (response.credential_id) {
      credential_id.value = response.credential_id
      otpView.value = true
    }
    else {
      await fetch()
    ...
    errMsg.value = err.data?.message || err.message
  }
  finally {
    loading.value = false
  }
    ...
    >
      <template #footer>
        ...
        <UAlert
          ...
          variant="soft"
          color="success"
          icon="i-hugeicons:checkmark-square-02"
          :title="confirmMsg"
          class="mt-4"
        />
      </template>
...
const schema = z.object({
  name: z.string(t('auth_name_required')).min(2, t('auth_name_min')),
  email: z.email(t('auth_email_invalid')),
  password: z.string(t('auth_password_required')).min(8, t('auth_password_min')),
  confirmPassword: z.string(t('auth_confirm_password_required')).min(8, t('auth_confirm_password_min')),
...
### Path: app/components/OTP.vue
```vue
<script setup lang="ts">
const toast = useToast()
const loading = ref(false)
const otp = ref<string[]>([])
const credential_id = defineModel<string>('credential_id', { required: true })
const errMsg = ref('')

async function confirmOtp() {
  try {
    loading.value = true
    await $fetch('/api/auth/confirm-otp', {
      method: 'POST',
      body: {
...
<template>
  <UPageCard
    ...
    description="Please check your email for the OTP code and enter it below to confirm your email address."
    variant="outline"
...
### Path: app/components/StarsBg.vue
```vue
<template>
